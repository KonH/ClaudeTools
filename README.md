...
- **`k` plugin** — common skills, invoked with the `k:` prefix:
  ...
  - `/k:specify` — capture feature intent and acceptance criteria as `docs/specs/<index>_<name>/spec.md`, stops for approval
  - `/k:plan` — turn an approved spec (or a purely technical task) into `docs/specs/<index>_<name>/plan.md`, gated by a required `docs/constitution.md` check
